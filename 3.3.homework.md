...
Значит echo Hi не выполнится.
  ...
  Имеет смысл использовать && потому что
  например:
...
vagrant@vagrant:~$ ps -eo stat | grep S | wc -l
56

vagrant@vagrant:~$ ps -eo stat | grep -E 'D|R|T|t|W|X|Z'  | wc -l
2

vagrant@vagrant:~$ ps -eo stat | grep t | wc -l
0       
